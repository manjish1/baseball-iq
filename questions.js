/* ============================================================
   ...
   A smaller hand-written set covers special rules that do not
   reduce to simple combinatorics (force-outs negating runs,
   cutoff-man reads, this league no-stealing/no-bunting/no-IFR
...
// A fielder standing right on the base he needs does not throw there — he just
...
// ---------- DEFENSE: ground ball "where is the play" ----------
// Real coaching note: tiny kids cannot reliably make a long throw across the
// diamond (like 3B or deep SS to 1st). When there is no force, young players
...
// do not have this problem even for young kids.
...
function genDefenseGroundBall() {
  const items = [];
  const templates = [
    (rd, od, fl) => `You are on defense. ${rd}, ${od}. A ground ball is hit to the ${fl}. Where is the play at?`,
    (rd, od, fl) => `${rd}, ${od}. You are on defense. The ${fl} fields a grounder. Where is the play?`
  ];
        ...
        for (const tmpl of templates) {
          if (outs === 2) {
            ...
            const explanation = validBases.length > 1
              ? `With 2 outs, any out ends the inning, so take whichever base is easiest.`
            ...
            const explanation = `Nobody is on base yet, so 1st is the only play, and you always make this out.`;
            ...
            push(combo, outs, fielder, tmpl, oc === 1 ? 1 : 2, HOLD_TEXT,
              ...
              `Nobody is forced to run here, and that is a long throw across the diamond, so hold onto the ball instead of risking an overthrow.`);
          } else {
            // Short, easy throw (pitcher/2nd baseman) — safe even for young players.
            // (1st baseman also lands here when there is a runner on base and
            // no force: he does not throw to his own bag, he just steps on it.)
            const correctText = baseActionText('1st', fielder);
            const explanation = `1st base is open, so nobody else is forced to run, and 1st base is the safe out.`;
...
// standing on — saying "You are on 1st. A runner is on 1st." is confusing
...
// ---------- OFFENSE: ground ball, runner decision ----------
function genOffenseGroundBall() {
  ...
  const templates = [
    (base, other, od, fl) => `You are on ${base}, ${od}.${other ? ' ' + other + '.' : ''} A ground ball is hit to the ${fl}. What do you do?`,
    (base, other, od, fl) => `${od}. You are on ${base}${other ? ', and ' + other.charAt(0).toLowerCase() + other.slice(1) : ''}. The ${fl} fields a grounder. What is your move?`
  ];
  // "Run to {next base}" and "run because you are forced/it is 2 outs" describe
  // the exact same physical action (sprint to the next base right now) — they
  // are not two different answers, just two justifications for one action. So
  // there is a single "run" answer, always phrased as the destination, and the
        ...
        for (const fielder of FIELDERS.slice(0, 3)) {
          for (const tmpl of templates) {
            ...
            if (outs === 2) {
              ...
              explanation = `With 2 outs, always run hard, since there is nothing to lose.`;
              level = 2;
            } else if (forced) {
              correctText = OPT_RUN(base.next);
              ...
              explanation = `No one is making you run, so only go if it is clearly safe.`;
              level = oc === 1 ? 2 : 3;
            }
...
function genOffenseFlyBall() {
  ...
  // does — so there is nothing to decide "after" the catch. The real decision
  // is AT CONTACT, before anyone knows if it will be caught, which is why that
  // version is phrased differently.
  const templatesCaught = [
    (base, od, of) => `You are on ${base} base, ${od}. A fly ball to ${of} is caught. What do you do?`,
    (base, od, of) => `${od}. Runner on ${base}, a fly ball to ${of} gets caught. What is the right move?`
  ];
  const templatesContact = [
    (base, od, of) => `You are on ${base} base, ${od}. A fly ball is hit to ${of}. What do you do right at contact?`,
    (base, od, of) => `${od}. Runner on ${base}, a fly ball is hit to ${of}. What is your move the instant it is hit?`
  ];
      ...
      for (const of of OF) {
        ...
        for (const tmpl of templates) {
          ...
          if (outs === 2) {
            ...
            explanation = `With 2 outs, a caught fly ends the inning no matter what you do, so break hard at contact. If it drops, you have a head start.`;
            level = 2;
          } else {
            correctText = OPT_TAG;
    ...
    for (const outs of [0, 1]) {
      for (const of of OF) {
        ...
        items.push(makeItem({
          ...
          prompt: `You are on defense. Runner on ${base.label}, ${OUTS_DESC[outs]}. A fly ball to ${of.label} is caught and the runner tags up. Where should the throw go?`,
          correctText, distractors,
          explanation: `The throw from an outfielder always goes through the cutoff man first, and he decides if it keeps going to ${base.dest === 'home' ? 'home plate' : base.dest + ' base'} or gets held up.`
        }));
      }
    }
  }
...
// ---------- DEFENSE: outfield coverage — who is the cutoff man, who covers 2nd ----------
...
function genDefenseCoverage() {
  ...
  const FLAVORS = [
    (zoneLabel, rd) => `You are on defense. ${rd}. A single is hit to ${zoneLabel}.`,
    (zoneLabel, rd) => `You are on defense. ${rd}. The batter rips one out to ${zoneLabel}.`
  ];
    ...
    for (const flavor of FLAVORS) {
      for (const rd of FLAVOR_RUNNERS) {
        ...
        items.push(makeItem({
          ...
          prompt: `${flavor(zone.label, rd.desc)} Who is the cutoff man for the throw?`,
...
function genOutfieldFundamentals() {
  ...
  const backupTemplates = [
    (who, fl, baseLabel) => `You are the ${who}. The ${fl} fields a grounder and throws to ${baseLabel}. What is your job?`,
    (who, fl, baseLabel) => `You are playing ${who}. The ${fl} fields a ground ball and throws over to ${baseLabel}. What should you be doing?`
  ];
  ...
  const gapTemplates = [
    (who, gapLabel) => `You are the ${who}. The ball is hit into ${gapLabel}. What do you do?`,
    (who, gapLabel) => `You are playing ${who} and the ball is hit to ${gapLabel}. What is your move?`
  ];
  ...
  for (const g of GAP_BACKUPS) {
    for (const tmpl of gapTemplates) {
      items.push(makeItem({
        ...
        explanation: `You are not the one making the play, so hustle over to back up your teammates in case the ball gets past them.`
      }));
    }
  }
  ...
  const popupTemplates = [
    (fl) => `You are the ${fl}, and a popup goes up near you. What should your very first step be?`,
    (fl) => `You are playing ${fl}. A popup is hit near you. What is the right first move?`
  ];
  ...
  for (const pos of POPUP_POS) {
    for (const tmpl of popupTemplates) {
      items.push(makeItem({
        ...
        explanation: `Always take your first step back on a popup, since it is a lot easier to come in for a short ball than to backpedal for one that sails over your head.`
      }));
    }
  }
...
const SPECIAL_SCENARIOS = [
  {
    ...
    prompts: [
      "You are on defense. Runners on 1st and 2nd, less than 2 outs. The batter hits an easy popup over the infield. What is the call?",
      "You are on defense, less than 2 outs, runners on 1st and 2nd. An easy popup goes up over the infield. What happens?"
    ],
    ...
    explanation: "This league has NO Infield Fly Rule, so it is just a regular fly ball. If it is dropped, nobody is out and the force is back on."
  },
  {
    level: 3, category: 'defense', outs: 1, runners: { first:true, second:true, third:true },
    hit: { type:'fly', pos:'SS', label:'Infield popup' },
    prompts: [
      "You are on defense. Bases loaded, 1 out. The batter hits an easy popup over the infield. What is the call?",
      "You are on defense, 1 out, bases loaded. An easy popup goes up over the infield. What is the ruling?"
    ],
    correctText: "No special rule, it must be caught for an out",
    distractors: ["Infield Fly, batter is automatically out", "It is a force play only", "Ground rule double"],
    explanation: "No Infield Fly Rule applies in this league, even with the bases loaded, so the defense has to actually catch it."
  },
  {
    level: 3, category: 'offense', outs: 0, runners: { first:true, second:true, third:false },
    hit: { type:'fly', pos:'2B', label:'Infield popup' },
    prompts: [
      "You are a runner on 1st or 2nd. The batter pops one up over the infield. What should you do?",
      "You are one of the runners on 1st or 2nd. An easy popup goes up over the infield. What is the right move?"
    ],
    correctText: "Stay near your base and be ready to tag up",
    distractors: ["Run to the next base right away, the batter is automatically out", "Both run home", "Ignore the play"],
    explanation: "There is no Infield Fly Rule here, so the batter is not out until it is caught, so do not wander off your base."
  },
  {
    level: 5, category: 'defense', outs: 0, runners: { first:true, second:false, third:true },
    hit: null,
    prompts: [
      "You are on defense. Runners on 1st and 3rd, no outs. You field a grounder and freeze the runner on 3rd by threatening to throw there, then instead throw to 2nd to get the runner from 1st. What happens to the runner on 3rd?",
      "You are on defense, no outs, 1st and 3rd. You hold the ball on the runner at 3rd, then throw to 2nd for the runner coming from 1st instead. What about the runner on 3rd?"
    ],
    correctText: "He is free to run home",
    distractors: ["He is stuck at 3rd no matter what", "He is automatically out", "Play stops immediately"],
    explanation: "This league has a freeze rule: once the defense makes a play on a different runner, the frozen runner is live again and can try to advance."
  },
  {
    level: 4, category: 'defense', outs: 0, runners: { first:false, second:true, third:false },
    hit: { type:'ground', pos:'CF', label:'Single to CF' },
    prompts: [
      "You are the outfielder. Runner on 2nd, no outs. A single to center field scores him. Who do you throw to?",
      "You are the outfielder, no outs, runner on 2nd scores on your single to center. Where should your throw go first?"
    ],
  ...
  },
  {
    ...
    prompts: [
      "You are the cutoff man. Runners on 1st and 3rd, no outs. A single to right scores the run and the other runner takes 2nd. What do you do?",
      "You are the cutoff man, no outs, 1st and 3rd. A single to right scores the run. What is your job?"
    ],
    correctText: "Line up for a cutoff throw",
    distractors: ["Stand at 2nd base", "Stand in the outfield", "Stand behind home"],
    explanation: "If the run cannot be stopped, the cutoff man can still get the other runner."
  },
  {
    level: 3, category: 'offense', outs: 0, runners: { first:false, second:true, third:false },
    hit: null,
    prompts: [
      "You are the batter. Runner on 2nd, no outs. You want to help move the runner to 3rd. What is your plan?",
      "You are the batter, no outs, runner on 2nd. You want to advance the runner. What is your plan?"
    ],
    correctText: "Hit away, bunting is not allowed in this league",
  ...
  },
  {
    ...
    prompts: [
      "You are on defense. Runner on 1st, no outs. A line drive is caught by the shortstop. What is the best play next?",
      "You are on defense. No outs, runner on 1st takes off as a line drive is caught by the shortstop. What is next?"
    ],
  ...
  },
  {
    ...
    hit: { type:'line', pos:'SS', label:'Line drive (caught)' },
    prompts: [
      "You are the runner on 1st, 1 out. A hard line drive is caught by the shortstop. What should you do?",
      "You are on 1st, 1 out. A line drive gets caught by the shortstop. What now?"
    ],
    correctText: "Get back to 1st fast",
    distractors: ["Keep running to 2nd", "Stop and watch", "Run to 3rd"],
    explanation: "If you left too early, hurry back before you are doubled off."
  },
  {
    level: 4, category: 'offense', outs: 0, runners: { first:true, second:false, third:false },
    hit: null,
    prompts: [
      "You are the runner on 1st, no outs. The batter hits it deep to the gap, and it might be a hit or it might be caught. What is your first move?",
      "You are on 1st, no outs. Your teammate smacks one to the gap. What is your first move?"
    ],
    correctText: "Wait, then run if it is a hit",
    distractors: ["Sprint to 2nd right away", "Freeze on 1st", "Tag up like it is caught"],
  ...
  },
  {
    ...
    prompts: [
      "You are on defense. Bases loaded, 1 out. A fly ball is caught and the runner from 3rd tags up. What is your best play?",
      "You are on defense, 1 out, bases loaded. A fly ball is caught and 3rd tags for home. What do you do?"
    ],
    correctText: "Throw to the cutoff man",
    distractors: ["Throw to 1st", "Throw to 2nd", "Do not throw anywhere"],
    explanation: "The throw home from an outfielder always goes through the cutoff man first, so the defense can react."
  },
  {
    level: 3, category: 'offense', outs: 0, runners: { first:false, second:false, third:true },
    hit: null,
    prompts: [
      "You are the runner on 3rd, no outs. You are rounding for home and the catcher is blocked up waiting for the ball right in front of the plate. What must you do?",
      "You are on 3rd, no outs, heading for home. The catcher is set up in front of the plate with the ball coming. What are you required to do?"
    ],
    correctText: "Slide to avoid the catcher",
    distractors: ["Run through him", "Jump over him", "Stop and go back to 3rd"],
    explanation: "This league requires runners to slide on plays at 2nd, 3rd, and home, so you cannot just run into the fielder."
  },
  {
    level: 2, category: 'offense', outs: 0, runners: { first:true, second:false, third:false },
    hit: null,
    prompts: [
      "You are the runner on 1st, no outs. The pitcher is set and about to deliver the pitch. What are you allowed to do?",
      "You are on 1st, no outs, pitcher is about to throw. What can you legally do right now?"
    ],
    ...
    explanation: "No leading off and no stealing in this league, so your foot cannot leave the base until the ball is hit."
  },
  {
    level: 5, category: 'offense', outs: 0, runners: { first:true, second:false, third:true },
    hit: null,
    prompts: [
      "You are the runner on 3rd, no outs, with a runner also on 1st. The defense freezes you near the bag, then turns and throws to 2nd to get the runner from 1st instead. What should you do?",
      "You are on 3rd, no outs, with a runner also on 1st. The defense holds you at 3rd, then throws to 2nd for the other runner. What is your move?"
    ],
    correctText: "Take off for home, you are free now",
    distractors: ["Stay frozen at 3rd no matter what", "You are automatically out", "Wait for the umpire to wave you on"],
    explanation: "Once the defense makes a play on a different runner, you are no longer frozen and can try to score."
  },
  {
    level: 5, category: 'offense', outs: 0, runners: { first:false, second:true, third:false },
    hit: null,
    prompts: [
      "You are rounding the bases and, in the excitement, you miss stepping on 2nd base but keep going and touch 3rd. What happens?",
      "You round the bases, skip over 2nd base by accident, and touch 3rd instead. What is the result?"
    ],
    correctText: "The umpire can call you out, no appeal needed",
    distractors: ["Nothing, unless the defense appeals it", "You are safe since nobody complained", "You just have to go back and re-touch 2nd"],
    explanation: "In this league, missing a base is a judgment call the umpire makes on their own, so the defense does not even need to appeal it like in regular baseball."
  },
  {
    level: 3, category: 'offense', outs: 0, runners: { first:false, second:true, third:false },
    hit: null,
    prompts: [
      "You are the runner on 2nd trying to beat a tag at 3rd. Are you allowed to slide in headfirst?",
      "You are on 2nd, sliding into 3rd to beat the throw. Can you go in headfirst?"
    ],
    correctText: "No, headfirst is only allowed going back to a base",
    distractors: ["Yes, headfirst is always fine", "Only if you are the tying run", "Only on the last out of the inning"],
    explanation: "Headfirst slides are not allowed when advancing to a base in this league, so feet first only. Headfirst is only okay when diving back to a base."
  },
  {
    level: 5, category: 'defense', outs: 0, runners: { first:true, second:true, third:false },
    hit: { type:'fly', pos:'3B', label:'Popup near foul line' },
    prompts: [
      "You are on defense. Runners on 1st and 2nd, no outs. A popup drifts near the foul line and your fielder lets it drop untouched. What happens if it lands fair?",
      "You are on defense. A popup drifts toward the line, runners on 1st and 2nd. Your fielder lets it fall untouched and it lands in fair territory. What is the result?"
    ],
    correctText: "It is just a live fair ball, nobody is out",
    distractors: ["The batter is automatically out", "Automatic double play", "It is a foul ball no matter what"],
    explanation: "No Infield Fly Rule applies here, so letting it drop does not create an out by itself. It is a live ball, and the defense still has to make a play."
  },
  {
    level: 5, category: 'offense', outs: 1, runners: { first:false, second:true, third:true },
    hit: { type:'ground', pos:'3B', label:'Sharp ground ball' },
    prompts: [
      "You are the runner on 3rd, with a runner also on 2nd, 1 out, 1st base open. A hard grounder is fielded deep by the third baseman. What do you do?",
      "You are on 3rd, 1 out, a runner also on 2nd, 1st open. A sharp grounder is fielded by the third baseman. What is your read?"
    ],
    correctText: "Go only if it is safe",
    distractors: ["Always run home", "Always stay", "Run to the dugout"],
    explanation: "No one is forcing you, so do not run into an easy out."
  },
  {
    level: 5, category: 'defense', outs: 1, runners: { first:true, second:true, third:false },
    hit: { type:'line', pos:'1B', label:'Line drive (caught)' },
    prompts: [
      "You are on defense. Runners on 1st and 2nd, 1 out. A line drive is caught by the first baseman and both runners had taken off. What is the smartest play?",
      "You are on defense. 1 out, 1st and 2nd both running on a line drive that is caught at first. What is your best play?"
    ],
  ...
  },
  {
    ...
    hit: null,
    prompts: [
      "You are on defense. Runner on 2nd, no outs. Should your corner infielders crash in expecting a bunt?",
      "You are on defense, no outs, runner on 2nd. Do you need to guard against a bunt here?"
    ],
    correctText: "No, bunting is not allowed in this league",
  ...
  },
  {
    ...
    prompts: [
      "You are on defense. Runners on 1st and 3rd, 2 outs. A ground ball goes to the second baseman. Where is the play, and does the run from 3rd count?",
      "You are on defense, 2 outs, 1st and 3rd. A grounder is hit to second. If you get the force, does the run from 3rd still count?"
    ],
    correctText: "Throw to 1st, run does not count",
    distractors: ["Throw home instead", "Throw to 3rd", "The run always counts"],
    explanation: "If the last out is a force play, any run on that play does not count."
  },
  {
    level: 5, category: 'offense', outs: 2, runners: { first:true, second:false, third:true },
    hit: { type:'ground', pos:'2B', label:'Ground ball' },
    prompts: [
      "You are the batter-runner. Runners on 1st and 3rd, 2 outs. You hit a ground ball to second and get forced out at 1st. Does the run from 3rd count?",
      "You are the batter-runner, 2 outs, 1st and 3rd. You are forced out at 1st on a grounder. Does the run from 3rd count?"
    ],
    correctText: "No, the run does not count",
    distractors: ["Yes, it always counts", "Only if he is fast", "The umpire decides"],
  ...
  },
  {
    ...
    prompts: [
      "You are on defense. Bases loaded, 1 out. A fly ball to right field is caught. Every runner tags up. Where should your throw go?",
      "You are on defense, 1 out, bases loaded, a fly ball to right is caught and all three tag up. Where should your throw go?"
    ],
    correctText: "Throw to the cutoff man",
    distractors: ["Throw to 2nd", "Throw to 1st", "Do not throw anywhere"],
    explanation: "The throw from the outfield goes through the cutoff man, who then decides where it needs to go, since stopping the run matters most."
  },
  {
    level: 4, category: 'offense', outs: 0, runners: { first:false, second:true, third:false },
    hit: null,
    prompts: [
      "You are the runner on 2nd. A throw to 1st sails past the first baseman but stays in play near the fence. What can you do?",
      "You are on 2nd. An overthrow at 1st stays in play by the fence. What is your move?"
    ],
    correctText: "Try to advance, but it is at your own risk",
    distractors: ["Nothing, the ball is automatically dead", "You must stay on 2nd", "You are awarded 3rd for free"],
    explanation: "If an overthrow stays in the field of play, runners can try to advance, but the defense can still make a play, so it is not a free base."
  },
  {
    level: 2, category: 'offense', outs: 0, runners: { first:false, second:false, third:false },
    hit: null,
    prompts: [
      "You are the runner rounding 3rd base and your coach gives you a stop sign. What should you do?",
      "You are rounding third, and your coach puts both hands up (stop sign). What is your move?"
    ],
    correctText: "Stop at 3rd",
    distractors: ["Run home anyway", "Slow down but keep going", "Go back to 2nd"],
    explanation: "Your coach can see the ball better than you can, so trust the stop sign."
  },
  {
    level: 2, category: 'defense', outs: 0, runners: { first:false, second:false, third:true },
    hit: { type:'ground', pos:'P', label:'Comebacker' },
    prompts: [
      "You are on defense. Runner on 3rd only, no outs. A slow roller is fielded by the pitcher. What is the play?",
      "You are on defense, no outs, runner on 3rd. The pitcher fields a comebacker. Where is the safe out?"
    ],
    correctText: "Throw to 1st",
    distractors: ["Throw home", "Throw to 3rd", "Throw to 2nd"],
    explanation: "The runner on 3rd is not forced, so take the easy out at first."
  }
];
